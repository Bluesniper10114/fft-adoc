...
function showHelpTip(e, id, sHtml, bHideSelects) {
  ...
  // position tooltip
  helpTipHandler.positionToolTip(e, id);
  ...
  // add listener on the document click Event - To hide present HelpTip
  document.body.onclick = function (w3cEvent) {
    if (el._helpTip) {
      var target = w3cEvent.target || w3cEvent.srcElement;
      var className = target.className;
      if (className != "helptip" && className.indexOf("helptip") < 0) {
        helpTipHandler.hideHelpTip(el);
      }
    }
  };
...
var helpTipHandler = {
  ...
  positionToolTip:  function (e, id) {
    ...
    var osh         = d.offsetHeight;
        var osw         = d.offsetWidth;
        var elem        = document.getElementById(id);
        var pos         = this.truePosition(elem);
        var trueBody    = (document.compatMode && document.compatMode!="BackCompat") ? document.documentElement : document.body;
        sOffsets      = this.getScrollOffsets();
        scrollTop   = sOffsets[1];
        scrollLeft  = sOffsets[0];

        d.style.left  = Number(parseInt(trueBody.clientWidth+scrollLeft) < parseInt(osw+pos[0]) ? Math.abs(parseInt((trueBody.clientWidth+scrollLeft) - osw)) : pos[0]) + 30 + "px";
        d.style.top   = Number(parseInt(trueBody.clientHeight+scrollTop) < parseInt(osh+pos[1]+elem.offsetHeight+2) ? Math.abs(parseInt(pos[1] - (osh + 2))) : Math.abs(parseInt(pos[1] + elem.offsetHeight + 2))) + "px";

    d.style.visibility = "visible";
  },

  truePosition : function(e) {
    var pos = this.cumulativeOffset(e);
    if(window.opera) { return pos; };
        var iebody      = (document.compatMode && document.compatMode != "BackCompat")? document.documentElement : document.body;
        var  dsocleft    = document.all ? iebody.scrollLeft : window.pageXOffset;
        var  dsoctop     = document.all ? iebody.scrollTop  : window.pageYOffset;
        var  posReal     = this.realOffset(e);
